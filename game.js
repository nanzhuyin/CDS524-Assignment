...
const loadModelFileBtn = document.getElementById("load-model-file-btn");
const loadModelFileInput = document.getElementById("load-model-file");
...
function applyModelSnapshot(snapshot, label = "外部模型") {
  const qTable = hydrateQTable(snapshot?.ai?.qTable);
  game.ai.qTable = qTable;
  game.ai.epsilon = clamp(toFiniteNumber(snapshot?.ai?.epsilon, game.ai.epsilon), 0, 1);
  game.ai.alpha = clamp(toFiniteNumber(snapshot?.ai?.alpha, game.ai.alpha), 0, 1);
  game.ai.gamma = clamp(toFiniteNumber(snapshot?.ai?.gamma, game.ai.gamma), 0, 1);
  game.ai.explored = Math.max(0, Math.floor(toFiniteNumber(snapshot?.ai?.explored, game.ai.explored)));
  game.ai.exploited = Math.max(0, Math.floor(toFiniteNumber(snapshot?.ai?.exploited, game.ai.exploited)));

  game.episode = Math.max(1, Math.floor(toFiniteNumber(snapshot?.stats?.episode, game.episode)));
  game.playerWins = Math.max(0, Math.floor(toFiniteNumber(snapshot?.stats?.playerWins, game.playerWins)));
  game.enemyWins = Math.max(0, Math.floor(toFiniteNumber(snapshot?.stats?.enemyWins, game.enemyWins)));
  game.trainingHistory.episodeReturns = Array.isArray(snapshot?.stats?.episodeReturns)
    ? snapshot.stats.episodeReturns.map((v) => toFiniteNumber(v, 0))
    : [];
  game.trainingHistory.outcomes = Array.isArray(snapshot?.stats?.outcomes)
    ? snapshot.stats.outcomes.map((v) => (toFiniteNumber(v, 0) > 0 ? 1 : 0))
    : [];
  game.trainingHistory.episodes = Array.isArray(snapshot?.stats?.episodes)
    ? snapshot.stats.episodes.map((item) => ({
      episode: Math.max(1, Math.floor(toFiniteNumber(item?.episode, 1))),
      reason: String(item?.reason ?? "unknown"),
      episodeReturn: toFiniteNumber(item?.episodeReturn, 0),
      epsilon: toFiniteNumber(item?.epsilon, game.ai.epsilon),
  ...
  gammaInput.value = game.ai.gamma.toFixed(3);
  game.modelMeta.loadedAt = new Date().toISOString();
  setModelStatus(`已加载 ${label} (Q状态 ${game.ai.qTable.size})`);
}
...
function loadModelFromFile(file) {
  ...
  reader.onload = () => {
    try {
      ...
      if (!snapshot?.ai?.qTable) {
        setModelStatus("文件格式不正确");
        return;
      }
      applyModelSnapshot(snapshot, file?.name || "本地模型");
    } catch (error) {
      setModelStatus("文件解析失败");
      console.error(error);
    }
  };
  reader.onerror = () => {
    setModelStatus("读取文件失败");
  };
...
if (loadModelFileBtn && loadModelFileInput) {
  loadModelFileBtn.addEventListener("click", () => {
    loadModelFileInput.click();
  });
  loadModelFileInput.addEventListener("change", (event) => {
